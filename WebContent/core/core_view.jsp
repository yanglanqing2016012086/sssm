      ...
      <table border="0" width="100%">
        ...
        <tr>
          ...
          <%//-----生成教师下拉框------
          	...
          	out.println(classtag.getTeacherTag(student_id));
          %>
          </td>
...
<%//-----用JavaBean查询出数据，并得到总记录条数------
	ResultSet rs=teachlession_select.teachlession_select_part(lession_id,student_id);
	rs.last();
...
<%
	String refName[]={"lession_id","student_id"};
	String refValue[]={lession_id+"",student_id+""};
...
%>
  ...
  <tr>
    ...
    <a href='teachlession_update1.jsp?teachlession_id=<%=rs.getLong("teachlession_id")%>&lession_id=<%=rs.getLong("lession_id")%>&student_id=<%=rs.getLong("student_id")%>'>修改</a>
    </td>
    <td align="center" width="176">
